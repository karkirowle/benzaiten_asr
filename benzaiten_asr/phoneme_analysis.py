import itertools
import benzaiten_asr.text as text
import benzaiten_asr.text.cmudict as cmudict_class
import pandas as pd
import numpy as np
from sklearn.metrics import confusion_matrix
from benzaiten_asr.per_utils import per_phoneme_per, align_sequences, afer
import os
from typing import List, Tuple, Union
from benzaiten_asr.utils import HParam
import re
import pkg_resources
from benzaiten_asr.default_configs import *
# It has only variables, so this is a relatively safe thing to do in this case
...
class AlternativeCMUDict():
    ...
    def __init__(self, location: str, conf):
        ...
        with open(location, encoding='utf-8') as file:
            ...
            for line in file:
                parts = line.split('  ')
                cmu_dict[parts[0]] = re.sub("[12345]","",parts[1].rstrip("\n"))
    ...
    def phonemes_in_dict(self):

        phoneme_set = set()
        for key in self.cmudict.keys():
            current_set = {phoneme.rstrip("12345") for phoneme in self.cmudict[key].split(" ")}
            #print(current_set)
            phoneme_set = phoneme_set.union(current_set)
...
class WERDetails:
    ...
    def __init__(self, location: str, skip_calculation=False, config: Union[None,str] =None):
        """
        A class aimed to isolate all functions regarding one WERDetails calculations

        :param location: path to wer_details/kaldi_mandarin_per_utt file
        :param skip_calculation: skip the phoneme alignments, will make the class mostly unusable
        :param config: either a configuration YAML including other parameters (see example) OR None (english Kaldi defaults)
        OR one of english_kaldi, english_espnet, dutch_kaldi
        """

        # The :param config part behaviour basically is implemented here
        if config is None:
            stream = pkg_resources.resource_filename(__name__, 'configs/eng.yaml')
        elif config in configs_dict.keys():
            path = configs_dict[config]
            stream = pkg_resources.resource_filename(__name__, path)
        else:
            stream = pkg_resources.resource_filename(__name__, config)

        # This is shared for all branches
        ...
        if config.phoneme.alternative_cmudict:
            self.cmudict = AlternativeCMUDict(package_dependent_dict,conf=self.config)
        else:
            self.cmudict = cmudict_class.CMUDict(package_dependent_dict)

        package_dependent_conversion = pkg_resources.resource_stream(__name__,
                                                                       os.path.join("csvs",
                                                                                    self.config.phoneme.conversion_mapping))

        # Converter table formatting
        self.converter_table = pd.read_csv(package_dependent_conversion)
    ...
    def all_pers_dataframe(self, ipa_conversion=False) -> pd.DataFrame:

        labels = sorted(list(set(self.all_ref_phonemes)))

        phonemes, counts = self.phoneme_count()

        # Convert phonemes

        if ipa_conversion:
            ipa_labels = [self.converter_table["IPA"][self.converter_table[self.config.phoneme.phoneme_name] == label].values[0] \
                          if label in self.converter_table[self.config.phoneme.phoneme_name] else label for label in labels]
            labels = ipa_labels

        # Line1: per per phoneme
        df = pd.DataFrame({ '0' :np.array([self.per_per_phoneme(phoneme) for phoneme in labels]),
                          # Line 2: phoneme count per honeme
                          '1': np.array([counts[phonemes.index(phoneme)] for phoneme in labels])},
                          labels)
        return df
    ...
    def all_moa_afers_dataframe(self) -> pd.DataFrame:

        moas = self.config.phoneme.moa
        values = [self.afer_per_phoneme(self.moa_to_phonemes(moa)) for moa in moas]
        df = pd.DataFrame(values, moas)
        return df
    ...
    def all_poa_afers_dataframe(self) -> pd.DataFrame:

        poas = self.config.phoneme.poa
        values = [self.afer_per_phoneme(self.poa_to_phonemes(poa)) for poa in poas]
        df = pd.DataFrame(values, poas)
    ...
    def clean_non_words(sentence: list) -> list:
        ...
        #for word in sentence:
            ...
            #print(word, regex.search(word))
        words = list(filter(lambda word: regex.search(word) is None, sentence))
